...
/**
 * Extract text signals from an image using Anthropic multimodal messages.
 * This is intentionally lightweight and used as best-effort OCR fallback.
 *
 * @returns {{ text: string, usage: object, model: string, completionId: string|null }}
 */
export async function extractImageTextAnthropic({
  imageBase64,
  mediaType,
  prompt,
  model,
  ...
  const resolvedPrompt = prompt ||
    "Extract readable study text from this image. Return plain text only. Include headings, bullet points, formulas, and short labels when visible.";
